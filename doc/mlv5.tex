...
\vspace{-17mm}
{\Large \bf Midlevel Prosodic Features and }

\smallskip
{\Large \bf Prosody Principal Components Analysis (PPCA)  Toolkit}
\medskip
...
\end{itemize}
It contains a a novel set of prosodic features that are robust,
...
\begin{itemize}\setlength{\itemsep}{0pt}\setlength{\parskip}{0pt}
\item Matlab (tested on release r2015a, but reported to also work,
  with minor modifications (e.g. a download of a freeware version of
  strsplit), on earlier release)
...
so this should create a loadings file; if so, you should be good to go.
...
three things: 1) convert it to the right format, 2) create a file
inventorying your audio, and 3) select a feature set to use.  Section
\ref{file-formats} has more information. 
...
distributional analysis, predicting actions from prosody, and
examining non-native dialog patterns
\cite{pca-lm,prosody-ir,sigdial-codec,ward-gaze,dimensions-uh-huh,l2english}.
Before reading on it's probably best to look at one of these papers
for an overview of the approach and the features.
...
\begin{figure*}[tp]
 \centerline{ 
 \includegraphics[width=11.4cm, trim = 1.cm 4.9cm 0cm 1.5cm, clip=true]{workflow-overview}
}
...
is memory-intensive.  If you're processing a lot of data, then the
rotation should be computed on just a subset, then applied to it all.
If this is not done, Matlab will gobble memory and freeze or crash the
machine.
...
primary track, left or right, as specified in the tracklist file, and
``inte'' to the other track, with the interlocutor's voice.
...
features can include NaNs, this is not appropriate for the mid-level
features.
...
In such cases the function should return other non-obtrusive value in
...
interpretable as patterns of behavior.  There are three ways to do
this interpretation, as described in this section.  

First, however, you may wish to look at the variance and cumulative
variance explained by the PCA-found dimensions.  For this: 
...
\begin{verbatim}
   load rotationspec;
   diagramDimensions(coeff, 'xxx.fss');
\end{verbatim}
...
rotationspec.  (If you don't remember, look at the value of {\tt
  rotation\_provenance} after you load the rotationspec.)
...
to 5 seconds before a desired timepoint, andthen play this region, and
...
their behavior with respect to the dimensions may be informative. 

One way is to examine the summary statistics ({\tt summary-stats.txt})
written automatically by {\tt applynormrot.m}, for two different populations.
...
populations, on the various dimesions, and eyeball them.  For example:
...
well as Linux.  This is the version described by this document.
