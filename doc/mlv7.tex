...
conveniently in Matlab and very well documented.

The second is David Talkin's Reaper.  The reason I added it was
because it can handle the Switchboard files that fxrapt cannot.  (I'm
guessing that the problem is due to recording issues with some files,
as discussed in {\tt
  https://catalog.ldc.upenn.edu/docs/LDC97S62/swb1\_manual.txt} in
Section 13.)  Another advantage is that it is about 7 times faster.
...
integration with the midlevel toolkit is awkward. The bash code {\tt
  reaperize.sh} does what's needed in the context of processing
Switchboard files, and serves as an illustration of the necessary
steps.
...
raises the question of whether this behavior messes up the creaky
voice feature.  I haven't investigated this yet.
