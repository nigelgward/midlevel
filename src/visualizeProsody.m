function done = visualizeProsody( path,dimension )
...
FEATURE_TYPES = {{'vo', 'Volume'}, {'sr', 'Speaking Rate'}, {'hp', 'Hight Pitch'}, {'lp', 'Low Pitch'}, {'wp', 'Wide Pitch'},{'np','Narrow Pitch'},{'wp-np','Pitch Range'},{'hp-lp', 'Pitch Height'}};
...
for plotNumber = 1: NUMBER_OF_PLOTS
    switch plotNumber
        ...
        case 3 
            ...
            %plottingData{2}{featureType};
        case 4 
            featureType = getFeatureType('wp-np');
            printLabel(featureType, yReference); 
            plotBotSpeakers(plottingData, featureType, yReference, false);
            %plottingData{2}{featureType};
    end
...
function plottingData=loadDataFromFile(path, dimension)
    ...
    getDimension = sprintf('dimension%.2d.txt',dimension);
    ...
    for speakerType = 1:2
        featureIndex = getFeatureType('hp-lp');
        featureIndexR = getFeatureType('wp-np');
        thFeatureIndex = getFeatureType('hp');
        wpFeatureIndex = getFeatureType('wp');
        tlFeatureIndex = getFeatureType('lp');
        npFeatureIndex = getFeatureType('np');
        xValues = plottingData{speakerType}{thFeatureIndex}(1, :);
        xValuesR = plottingData{speakerType}{wpFeatureIndex}(1,:);
        yValues = plottingData{speakerType}{thFeatureIndex}(2, :) - plottingData{speakerType}{tlFeatureIndex}(2, :);
        yValuesR = plottingData{speakerType}{wpFeatureIndex}(2, :)- plottingData{speakerType}{npFeatureIndex}(2, :);
        plottingData{speakerType}{featureIndex} =  [xValues; yValues * TH_TL_GAIN]; % multiply y values with TH_TL_GAIN. otherwise final plot
        plottingData{speakerType}{featureIndexR} = [xValuesR; yValuesR *TH_TL_GAIN];                                                                            % shows very small values for th - tl.
    end %end of for
end %end of function
