...
OUTPUT_DIRECTORY = 'TestLoadingsVisualization';

% hp-lp graph is multiplied by HP_LP_GAIN
global HP_LP_GAIN;
HP_LP_GAIN = 1.5;
...
FEATURE_TYPES = {{'vo', 'Volume'}, {'sr', 'SpRate'}, {'hp', 'Hight Pitch'}, {'lp', 'Low Pitch'}, {'rf', 'Run Fract'}, {'ju', 'Jump'}, {'hp-lp', 'hp - lp'}};
...
function plottingData=loadDataFromFile(path, dimension)
    ...
    %compute values for hp-lp feature for each speaker
    for speakerType = 1:2
        featureIndex = getFeatureType('hp-lp');
        hpFeatureIndex = getFeatureType('hp');
        lpFeatureIndex = getFeatureType('lp');
        xValues = plottingData{speakerType}{hpFeatureIndex}(1, :);
        yValues = plottingData{speakerType}{hpFeatureIndex}(2, :) - plottingData{speakerType}{lpFeatureIndex}(2, :);
        plottingData{speakerType}{featureIndex} =  [xValues; yValues * HP_LP_GAIN]; % multiply y values with HP_LP_GAIN. otherwise final plot
                                                                                    % shows very small values for hp - lp.
    end %end of for
end %end of function
...
function done = plotArea(plottingData, yBase, speakerType, featureType)
    ...
    lineStyle = '-'; %default straight line
